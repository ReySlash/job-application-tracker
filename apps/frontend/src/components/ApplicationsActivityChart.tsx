...
function ApplicationsActivityChart(props: Props) {
  ...
  return (
    <section className="rounded border border-gray-200 bg-white p-4 shadow dark:border-slate-700 dark:bg-slate-900">
      <h2 className="text-xl font-semibold text-slate-900 dark:text-slate-100">Application Activity</h2>
      <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Applications submitted over the last 6 months.</p>
        ...
        {data.map((item) => {
          ...
          return (
            <div className="flex h-full min-w-0 flex-1 flex-col items-center justify-end" key={item.label}>
              <span className="mb-2 text-sm font-medium text-slate-700 dark:text-slate-200">{item.count}</span>
              <div className="flex h-40 w-full max-w-12 items-end rounded bg-gray-100 dark:bg-slate-800">
                <div
              ...
              <span className="mt-2 text-xs text-slate-500 dark:text-slate-400">{item.label}</span>
            </div>
          );
        })}
      </div>
